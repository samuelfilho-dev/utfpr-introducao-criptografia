...
def matching (freq):
   '''
   A função `matching` recebe um vetor de frequências de letras de um determinado alfabeto
   e retorna o melhor deslocamento para que as frequências se aproximem das frequências
   esperadas para o alfabeto em questão.

   @param freq: vetor de frequências de letras
   @return: best_rotate: melhor deslocamento
   '''
   best_fit = 1e100
   best_rotate = 0
   Sum = sum(elem for elem in freq)
   for rotate in range(0, alphabet_size, 1):
      fit = 0
...
def freq_every_nth (msg, interval):
   '''
   A função `freq_every_nth` quebra a cifra de Vigenère, descriptografando a mensagem
   através da analise da mensagem e o intervalo de uma possível chave que possa conter 30 caracteres.

   @param msg: mensagem
   @param interval: intervalo
   @return: ret: frequência de letras da mensagem
   @return: key: chave que melhor descriptografa a mensagem
   '''
   accu = alphabet_size * [0]
   key = ""
   for j in range(0, interval, 1):
      freq = alphabet_size * [0]
      for i in range(j, len(msg), interval):
